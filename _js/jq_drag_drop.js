DragDrop = {
	...
	setupElement: function (event) {
		...
		switch(setElementType) {
			...
			case 'text_box':
				...
				var inputBorderWidth = $selectedElement.find('input').style('border-width');
				var inputBorderWidth = parseInt(inputBorderWidth);
			...
			case 'text_area':
				...
				var inputBorderWidth = $selectedElement.find('textarea').style('border-width');
				var inputBorderWidth = parseInt(inputBorderWidth);
			...
			case 'dropdown':
				...
				var inputRequired = $selectedElement.find('select').prop('required');
				var inputMulti = $selectedElement.find('select').prop('multiple');

				var inputBorderStyle = $selectedElement.find('select').css('border-style');
				var inputBorderWidth = $selectedElement.find('select').style('border-width');
				var inputBorderWidth = parseInt(inputBorderWidth);
				var inputBorderRadius = $selectedElement.find('select').css('border-radius');
				var inputBorderRadius = parseInt(inputBorderRadius);
				...
				self.getElementTag(formListFile).done(function(data) {
					    ...
					    if ($("#text_box_multi_select_checkbox").length) {

							// console.log("RDO: " + inputReadonly); 
					  		$("#text_box_multi_select_checkbox").filter(function () {
								if(inputMulti){
					  				$(this).prop('checked', true);
						  		} else {
						  			$(this).prop('checked', false);
			...
			case 'image':
				...
				var inputBorderWidth = $selectedElement.find('img').style('border-width');
				var inputBorderWidth = parseInt(inputBorderWidth);
			...
			case 'file':
				...
				var subLabelValue = $selectedElement.find('label small').text();
				var instructionValue = $selectedElement.find('a').attr('title');
				var inputType = $selectedElement.find('input').attr('accept');
				var inputRequired = $selectedElement.find('input').prop('required');
				var labelAlignment = $selectedElement.find('label').attr('class').split(' ')[2]; //CHOOSING LABEL CLASS text-center [the 3rd class]
				var labelColor = $selectedElement.find('.control-label').css('color'); //CHOOSING LABEL STYLE color
			...
			case 'submit':
				...
				//COLLECT VALUES FROM DROPPED ELEMENT
				var labelValue = $selectedElement.find('input').val();
				var inputHeight = $selectedElement.find('input').css("height");
				var inputWidth = $selectedElement.find('input').css("width");

				var labelColor = $selectedElement.find('input').attr('class').split(' ')[1]; //CHOOSING LABEL CLASS text-center [the 3rd class]
				var labelAlignment = $selectedElement.find('input').attr('class').split(' ')[2]; //CHOOSING LABEL CLASS text-center [the 3rd class]

				var inputBorderStyle = $selectedElement.find('input').css('border-style');
				var inputBorderWidth = $selectedElement.find('input').style('border-width');
				// console.log("width: " + inputBorderWidth);
				var inputBorderWidth = parseInt(inputBorderWidth);
				// console.log("parseInt: " + inputBorderWidth);
				var inputBorderRadius = $selectedElement.find('input').css('border-radius');
				// console.log("radius: " + inputBorderRadius);
				var inputBorderRadius = parseInt(inputBorderRadius);
				// console.log("parseInt: " + inputBorderRadius);
				var inputBorderColor = $selectedElement.find('input').css('border-color');
				...
				self.getElementTag(formListFile).done(function(data) {
					    ...
					    if ($("#text_box_label_color").length) {              

					  		$("#text_box_label_color").filter(function() {
							    return $(this).val(labelColor); 
							}).attr('selected', 'selected');
					    ...
					    if ($("#text_box_border_color").length) {              

					  		$("#text_box_border_color").filter(function() {
							    return $(this).val(inputBorderColor); 
							}).attr('selected', 'selected');
	...
	textBoxSave: function (event) {
		...
		var newBorderThickness = newBorderThickness + "px";
	...
	dropdownSave: function (event) {
		...
		//SETTING BORDER STYLE
		$widgetBoxElement.find('select').css("border-style", newBorderStyle);
		$simpleElement.find('select').css("border-style", newBorderStyle);

		//SETTING BORDER TOP COLOR 
		$widgetBoxElement.find('select').css("border-top-color", newLabelColor);
		$simpleElement.find('select').css("border-top-color", newLabelColor);

		//SETTING BORDER RIGHT COLOR 
		$widgetBoxElement.find('select').css("border-right-color", newLabelColor);
		$simpleElement.find('select').css("border-right-color", newLabelColor);

		//SETTING BORDER BOTTOM COLOR 
		$widgetBoxElement.find('select').css("border-bottom-color", newLabelColor);
		$simpleElement.find('select').css("border-bottom-color", newLabelColor);

		//SETTING BORDER LEFT COLOR 
		$widgetBoxElement.find('select').css("border-left-color", newLabelColor);
		$simpleElement.find('select').css("border-left-color", newLabelColor);

		//SETTING BORDER WIDTH
		$widgetBoxElement.find('select').css("border-width", newBorderThickness);
		$simpleElement.find('select').css("border-width", newBorderThickness);

		//SETTING BORDER RADIUS
		$widgetBoxElement.find('select').style("border-radius", newBorderRadius,  "important");
		$simpleElement.find('select').style("border-radius", newBorderRadius, "important");
		...
		//SETTING MULTIPLE CHECKBOX
		if(newMultiCheckbox){
			$widgetBoxElement.find('select').attr('multiple', '');
			$simpleElement.find('select').attr('multiple', '');
		} else {
			$widgetBoxElement.find('select').removeAttr('multiple');
			$simpleElement.find('select').removeAttr('multiple');
		}

		//SETTING RQUIRED CHECKBOX
		if(newRequiredCheckbox){
			$widgetBoxElement.find('select').attr('required', 'required');
			$simpleElement.find('select').attr('required', 'required');
		}else {
			$widgetBoxElement.find('select').removeAttr('required');
			$simpleElement.find('select').removeAttr('required');
		}
	...
	submitSave: function (event) {
		...
		var newBorderStyle = $document.find('#text_box_border_style').val();
		var newBorderColor = $document.find('#text_box_border_color').val();
		var newBorderColor = "#" + newBorderColor;
		...
		//SETTING MODAL TEXT INTO WIDGET BOX		
		var labelValue = $widgetBoxElement.find('input').val(newLabelText);

		//SETTING MODAL TEXT INTO REGULAR DROPPED BLOCK
		var labelValue = $simpleElement.find('input').val(newLabelText);

		//SETTING DROPDOWN WIDTH
		$widgetBoxElement.find('input').css('width', newWidth);
		$simpleElement.find('input').css('width', newWidth);

		//SETTING DROPDOWN HEIGHT
		$widgetBoxElement.find('input').css('height', newHeight);
		$simpleElement.find('input').css('height', newHeight);
		...
		//SETTING BORDER TOP COLOR 
		$widgetBoxElement.find('input').style("border-top-color", newBorderColor, "important");
		$simpleElement.find('input').style("border-top-color", newBorderColor, "important");

		//SETTING BORDER RIGHT COLOR 
		$widgetBoxElement.find('input').style("border-right-color", newBorderColor, "important");
		$simpleElement.find('input').style("border-right-color", newBorderColor, "important");

		//SETTING BORDER BOTTOM COLOR 
		$widgetBoxElement.find('input').style("border-bottom-color", newBorderColor, "important");
		$simpleElement.find('input').style("border-bottom-color", newBorderColor, "important");

		//SETTING BORDER LEFT COLOR 
		$widgetBoxElement.find('input').style("border-left-color", newBorderColor, "important");
		$simpleElement.find('input').style("border-left-color", newBorderColor, "important");

		//SETTING BORDER WIDTH
		$widgetBoxElement.find('input').style("border-width", newBorderThickness);
		$simpleElement.find('input').style("border-width", newBorderThickness);
		...
		//SETTING LABEL COLOR 
		$widgetBoxElement.find('input').alterClass('btn-*', newLabelColor);
		$simpleElement.find('input').alterClass('btn-*', newLabelColor);

		//SETTING LABEL ALIGNMENT 
		$widgetBoxElement.find('input').alterClass('pull-*', newAlignment);
		$simpleElement.find('input').alterClass('pull-*', newAlignment);
	...
	fileSave: function (event) {
		...
		var subLabelValue = $widgetBoxElement.find('label small').text(newSubLabelText);
		var instructionValue = $widgetBoxElement.find('a').attr('title', newInstructionText);
		var inputType = $widgetBoxElement.find('input').attr('accept', newInputType);

		//SETTING MODAL TEXT INTO REGULAR HEADING BLOCK
		var labelValue = $simpleElement.find('label p').text(newLabelText);
		var subLabelValue = $simpleElement.find('label small').text(newSubLabelText);
		var instructionValue = $simpleElement.find('a').attr('title', newInstructionText);
		var inputType = $simpleElement.find('input').attr('accept', newInputType);
		...
		//SETTING INPUT COLOR 
		$widgetBoxElement.find('input').css('color', newLabelColor);
		$simpleElement.find('input').css('color', newLabelColor);
