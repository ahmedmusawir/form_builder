	...
	setupElement: function (event) {
		...
		switch(setElementType) {
			...
			case 'radio':
				...
				var inputHeight = $selectedElement.find('#radio_holder').attr('class').split(' ')[1]; //CHOOSING DIV's CLASS input-md [the 2nd class]
				var inputStyle = $selectedElement.find('#radio_holder > label').attr('class'); //CHOOSING DIV > LABEL's CLASS radio-inline [the only class]
